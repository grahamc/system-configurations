...
if not test -f /etc/fonts/conf.d/51-local.conf
  ...
  or echo -s (set_color red) 'ERROR: Unable to link the system configuration preset. Your system configuration will not be read by fontconfig.' (set_color normal) >/dev/stderr
end
