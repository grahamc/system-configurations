...
flock --nonblock /tmp/kitty-autoreload-lock --command "watchman-make --root '$kitty_config_path' --pattern '**/*' --run 'reload-kitty \"Configuration change detected, \" >/dev/tty' >/dev/null 2>/dev/null" >/dev/null &
disown
