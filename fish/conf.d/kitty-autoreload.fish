...
flock --nonblock /tmp/kitty-autoreload-lock --command "watchman-make --root '$kitty_config_path' --pattern '**/*' --run 'killall --signal SIGUSR1 kitty x-terminal-emulator' >/dev/null 2>/dev/null" >/dev/null &
disown
