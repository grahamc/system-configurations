...
# System dependencies
sudo apt install --yes build-essential procps curl file git
...
# Required for other packages like neovim
brew install gcc
...
python_command="$(command -v python3 >/dev/null 2>&1 && printf python3 || printf python)"
"$python_command" -c 'import resource as res; res.setrlimit(res.RLIMIT_NOFILE, (10000, res.getrlimit(res.RLIMIT_NOFILE)[1])); import subprocess as sp; sp.run(["brew", "bundle", "install", "--no-upgrade", "--file", "./brew/Brewfile"])'
