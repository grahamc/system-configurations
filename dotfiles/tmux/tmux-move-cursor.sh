...
# TODO: fails occasionally so I added `|| true` to avoid a tmux enter prompt. Probably related to the issue
# with `commandline` in the TODO above.
tmux send-keys -t "${session_id}:${window_id}.${pane_id}" -N "$move_count" "$direction" || true
...
# fi
# kill -USR2 "$pane_pid"
# cursor_index="$(cat /tmp/fish-tmux-cursor)"
# commandline="$(cat /tmp/fish-tmux-commandline)"
...
#     cur="$cursor_index"
#     limit=0
#     newlines_required="$((cursor_y - mouse_y + 1))"
...
#   else
#     cur="$cursor_index"
#     limit="$commandline_length"
#     newlines_required="$((mouse_y - cursor_y + 1))"
#     while [ "$cur" -ne "$limit" ] && [ "$newlines_required" -gt 0 ]; do
#       cur="$((cur + 1))"
#       if [ "${commandline:cur:1}" = "\n" ]; then
#         newlines_required="$((newlines_required - 1))"
