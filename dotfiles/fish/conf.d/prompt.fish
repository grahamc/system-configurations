...
function fish_prompt --description 'Print the prompt'
    ...
    if set --query TRANSIENT
        set --erase TRANSIENT
        printf $separator\n(set_color --reverse --italics brblack)'  '(path basename (prompt_pwd))(set_color normal)(set_color --reverse brblack)' ∙ '(set_color --reverse --italics brblack)(date +"%T")'  '$_color_normal
        return
