{ inputs, self, ... }:
  {
    perSystem = {lib, system, pkgs, ...}:
      let
        ...
        shellOutputs = optionalAttrs
          (builtins.elem system (with inputs.flake-utils.lib.system; [ x86_64-linux x86_64-darwin ]))
          (let
            hostName = "no-host";
            homeManagerOutput = self.lib.createHomeOutput system {
              inherit hostName;
              modules = [
                self.darwinModules.homeModules.profile.system-administration
