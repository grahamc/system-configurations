...
{...}: {
  perSystem = {
    ...
    in (mapAttrs (_hostName: output: output.activationPackage) homeManagerOutputsByHostName);

    nixDarwinPackagesByName = let
      nixDarwinOutputsByHostName = attrByPath ["legacyPackages" "darwinConfigurations"] {} self';
    in (mapAttrs (_hostName: output: output.system) nixDarwinOutputsByHostName);
