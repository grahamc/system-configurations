...
# can edit a file and have the changes applied instantly without having to switch generations.
{
  ...
  options.repository.symlink = let
    inherit (lib) types;
    target = lib.mkOption {
      type = types.str;
    };
    ...
    executableSymlinkOptions = submoduleContext @ {name, ...}: {
      options = {
        ...
        target = lib.mkOption {
          type = types.str;
          apply = value:
            if submoduleContext.config.recursive
  ...
  config = let
    ...
    convertAbsolutePathStringToPath = pathString: let
    ...
    convertFileToHomeManagerSymlink = file: let
      ...
      symlinkSource =
        if config.repository.symlink.makeCopiesInstead
